...
$sql = "select img from users where id=$tmp_id";
...
if ($_FILES['profile_image']['error'] === 0) {
	...
	$sql = "update users set full_name='$full_name',mobile=$mobile,img='$photo' where id=$tmp_id";
	mysqli_query($link, $sql) or die(mysqli_error($link));
} else if ($_FILES['profile_image']['error'] === 4) {
	$sql = "update users set full_name='$full_name',mobile=$mobile where id=$tmp_id";
	mysqli_query($link, $sql) or die(mysqli_error($link));
}
?>
<script>
